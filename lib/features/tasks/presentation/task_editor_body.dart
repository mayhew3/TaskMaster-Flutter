...
/// Data passed to a [TaskEditorBody.headerBuilder]. The body owns the
/// editor state; the host wrapper (full-screen route vs docked pane)
/// supplies only the header *chrome* via the builder, reading whatever
/// fields it needs from this object.
class TaskEditorHeaderInfo {
  /// "New task" / "Edit task".
  final String title;
  final bool isEditMode;

  /// Delete handler — non-null only in edit-mode. Wire to the header's
  /// delete affordance.
  final VoidCallback? onDelete;

  /// Close/back handler (same as the body's [TaskEditorBody.onClose]).
  final VoidCallback onClose;

  /// The task being edited, or `null` in add-mode. The docked pane reads
  /// `area` / `project` from it for its header strip.
  final TaskItem? taskItem;

  const TaskEditorHeaderInfo({
    required this.title,
...
/// The shared task-editor body — the field column plus the save / cancel /
/// delete / validation logic — extracted from `TaskAddEditScreen` (TM-384).
///
/// Two hosts consume it:
///   - `TaskAddEditScreen` — the full-screen route (phone); supplies a
///     `Scaffold` + gradient + back-chevron header. `onClose` / `onDeleted`
///     / `onSaveSucceeded` all pop the route. `useRootNavigatorForPickers`
///     is `true` (pickers behave as before TM-384).
///   - `DockedTaskEditorPane` — the wide right-pane editor; supplies the
///     pane chrome and provider-driven callbacks, and a nested `Navigator`
///     so the pickers (`useRootNavigatorForPickers: false`) render scoped
///     to the pane.
///
/// The body never calls `Navigator.pop` itself — every close/exit path
/// runs through an injected callback.
class TaskEditorBody extends ConsumerStatefulWidget {
  final String? taskItemId;

  /// When `true` and adding a new task (no [taskItemId]), pre-stamp the
  /// blueprint with the current user's `familyDocId`.
  final bool defaultFamilyShared;

  /// Invoked when the user closes the editor entirely — the header's
  /// back chevron (full-screen) or Close (X) icon (docked). On the
  /// full-screen route this pops; on the docked pane this clears the
  /// selection and collapses the right pane.
  final VoidCallback onClose;

  /// Invoked when the user taps Cancel in the sticky action bar.
  /// On the full-screen route this pops (same as `onClose`); on the
  /// docked pane this discards unsaved edits but leaves the editor
  /// open on the same task — symmetric with `onSaveSucceeded`, which
  /// also stays open. The body itself doesn't revert the blueprint;
  /// the host is expected to re-key the body (e.g. via a generation
  /// counter) so a fresh `State` re-reads from `taskProvider`.
  final VoidCallback onCancel;

  /// Invoked after a delete has been confirmed and persisted.
  final VoidCallback onDeleted;

  /// Invoked after a save succeeds, with the saved task's docId (the
  /// newly-generated docId in add-mode) AND the persisted [TaskItem]
  /// snapshot the success was detected from (null in add-mode — the
  /// new task's lookup is left to the caller's next provider read).
  /// The snapshot lets the docked pane re-key its child with the
  /// just-saved task directly, sidestepping the race where reading
  /// `taskProvider(docId)` on the new body's first build can return
  /// stale data before the Drift stream emit has propagated through
  /// Riverpod's family-invalidation chain.
  final void Function(String savedDocId, TaskItem? savedTask) onSaveSucceeded;

  /// `true` → pickers/dialogs target the root navigator (full-screen).
  /// `false` → they target the nearest (nested) navigator (docked pane).
  final bool useRootNavigatorForPickers;

  /// Builds the header chrome. Called from the body's data branch so the
  /// header can reflect edit/add mode and wire the body's delete handler.
  final Widget Function(BuildContext context, TaskEditorHeaderInfo info)
      headerBuilder;

  /// Optional override used by the docked editor's re-key path: when
  /// non-null, the body initialises directly from this snapshot instead
  /// of doing the deferred `ref.watch(taskProvider(taskItemId))` read.
  /// Eliminates a 50%-repro race after Save Changes where the freshly-
  /// mounted body would read `taskProvider(docId)` before Riverpod had
  /// propagated the save's stream emit, blueprinting from the pre-edit
  /// task and visually reverting the user's edits even though the list
  /// reflected the persisted values.
  final TaskItem? initialTaskOverride;

  const TaskEditorBody({
    super.key,
    this.taskItemId,
    ...
    required this.useRootNavigatorForPickers,
    required this.headerBuilder,
    this.initialTaskOverride,
  });

  @override
  ConsumerState<TaskEditorBody> createState() => _TaskEditorBodyState();
}

class _TaskEditorBodyState extends ConsumerState<TaskEditorBody> {
  // Instance (not static) so two editor bodies can never collide on the
  // same Form state — the docked pane and a full-screen route are layout-
  // exclusive today, but a static key would be a latent footgun.
  ...
  /// In add-mode, the in-flight `AddTask.call` future. `_scheduleAutoClose`
  /// awaits it to learn the newly-created task's docId before calling
  /// [TaskEditorBody.onSaveSucceeded].
  Future<String>? _pendingAddFuture;
  ...
  void didChangeDependencies() {
    ...
    if (!_initialized) {
      if (widget.taskItemId == null) {
        _initializeTask(null);
        _initialized = true;
      } else if (widget.initialTaskOverride != null) {
        // The docked editor's re-key-after-save path passes the
        // just-persisted snapshot here so the new body initialises
        // from it directly — bypasses the deferred `taskProvider`
        // read and its race with Drift-stream propagation.
        _initializeTask(widget.initialTaskOverride);
        _initialized = true;
      }
    }
  }
  ...
  void _initializeTask(TaskItem? task) {
    if (kDebugMode) {
      debugPrint(
        '[TaskEditorBody] _initializeTask docId=${task?.docId} '
        'priority=${task?.priority} duration=${task?.duration} '
    ...
    tasksAsync.whenData((tasks) {
      recurrencesAsync.whenData((recurrences) {
        ...
        if (editMode()) {
          ...
          if (kDebugMode) {
            debugPrint(
              '[TaskEditorBody] _checkForAutoClose '
              'latestTask=${latestTask?.docId} '
              'priority=${latestTask?.priority} '
              'recurUnit=${latestTask?.recurUnit} '
              'recurrenceDocId=${latestTask?.recurrenceDocId} | '
              'baseline priority=${taskItem!.priority} '
              'recurUnit=${taskItem!.recurUnit} '
              'recurrenceDocId=${taskItem!.recurrenceDocId} | '
              'hasTaskChanges=$hasTaskChanges',
            );
          }
          // Gate success on `hasTaskChanges` specifically — NOT
          // `hasTaskChanges || hasRecurrenceChanges` (the pre-fix gate).
          //
          // `UpdateTask.call` writes the recurrence row BEFORE the task
          // row, so on a save that touches both, the recurrence stream
          // emits first and triggers this listener with `latestTask`
          // still stale (pre-task-write) but `latestRecurrence` already
          // updated. The OR gate fired on that half-applied state and
          // re-keyed the editor body from the stale `latestTask` — all
          // of the user's edits appeared to revert in the pane even
          // though the list (via a different watcher path) showed the
          // persisted values. Repros intermittently in production
          // (~50%) when the user edits a recurrence-affecting field
          // (recurUnit / recurNumber / recurWait / anchor) AND a non-
          // recurrence task field in the same save.
          //
          // Gating on `hasTaskChanges` alone naturally waits past the
          // recurrence-first emit (where it's false) and fires on the
          // task emit (where it's true). Every UI-driven recurrence
          // change in this editor also mutates a task field (the unit/
          // number/wait/anchor are stored on the task row in addition
          // to the recurrence row), so this gate doesn't strand any
          // reachable save flow.
          if (hasTaskChanges) {
            popped = true;
            _scheduleAutoClose(savedTask: latestTask);
          }
        } else {
  ...
  /// Defer the success callback until after the current notification cycle
  /// completes. Multiple ref.watch / ref.listen subscribers depend on the
  /// same providers; firing synchronously would dispose the element mid-
  /// cycle (TM-348). In add-mode, awaits [_pendingAddFuture] so the
  /// callback receives the newly-created task's docId.
  ///
  /// [savedTask] is the persisted snapshot at success-detection time
  /// (non-null on the edit path; null on the add path — the new task's
  /// snapshot isn't directly available here, the caller can fetch it
  /// itself if it needs to). Passing it through to [TaskEditorBody.
  /// onSaveSucceeded] lets the docked editor re-key its child with the
  /// just-saved task directly, sidestepping the race where the new
  /// body's first `ref.watch(taskProvider(docId))` reads stale data
  /// before Riverpod has propagated the save's stream emit.
  ...
  void _onSavePressed() {
    ...
    if (form != null && form.validate()) {
      ...
      if (editMode()) {
        ...
        // Capture the future so `_scheduleAutoClose` can await the new
        // task's docId for the add → edit transition (TM-384 D5).
        _pendingAddFuture =
  ...
  Widget _buildEditor(BuildContext context, TimezoneHelper timezoneHelper) {
    return Column(
      children: [
