...
7. **Deferred review findings — [TM-372](https://mayhew3.atlassian.net/browse/TM-372).** Items surfaced by review passes that were triaged as real-but-non-blocking: sub-second precision window in the TaskDao back-fill comparison; in-memory-vs-SQL precision drift in the same back-fill's WHERE-clause guard; `BuiltSet → toSet()` allocation churn in `_DueStatusDropdown`; `ShowCompleted` startup-state prefetch race (Completed-already-visible on first build doesn't auto-prefetch the older-completed batch — the runtime off→on listener still works). None block this PR. The original `_initializeTask` priority-migration concern (also originally tracked here) was resolved in round-3 by scheduling the migration write via `WidgetsBinding.addPostFrameCallback`.
